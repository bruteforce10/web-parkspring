...
const FeatureList = ({ headingFeature, listFeature }) => {
  ...
  const transformedData = listFeature.map((item, index) => ({
    ...
    content: (
      <div className="flex h-full w-full items-center justify-center overflow-hidden rounded-md text-white">
        <img
          src={item?.image?.url}
          className="block h-auto w-full rounded-md object-contain sm:h-full sm:w-full"
